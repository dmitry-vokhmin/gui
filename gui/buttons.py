...
class Buttons(tk.Frame):
    def __init__(self, window, menu_point, *args, **kwargs):  # args, kwargs
    ...
    def show_buttons(self):
        show_all_button = tk.Button(self, text=f"Show all {self.menu_point.replace('_', ' ')}", width=20, height=2,
                                    command=self.window.show_data(self.window.web_api.get_data, f"{self.menu_point}"))
        ...
        add_button = tk.Button(self, text=f"Add {self.menu_point.replace('_', ' ')}", width=20, height=2,
                               command=self.window.show_form(f"{self.menu_point}"))
